...
import React, { useState, useEffect } from 'react'; // useEffect は今回直接は使わないかもしれませんが、汎用的に残します
import { db } from '../../services/firebase'; // firebase.tsのパスを確認
import { doc, updateDoc, Timestamp, serverTimestamp } from 'firebase/firestore';
import { UserWithId, Table } from '../../types'; // Table型は直接使わないかもしれませんが、他の機能で使われている可能性を考慮
// import { getFunctions, httpsCallable } from 'firebase/functions'; // 今回は直接Firestoreを更新
import { AiOutlineLoading } from 'react-icons/ai';
// import { format } from 'date-fns'; // このモーダル内では直接使わないためコメントアウトも可

// StatusBadge コンポーネント (変更なし、または共通化されていればインポート)
...
export interface UserDetailsModalProps {
  ...
  onBalanceResetSuccess?: (userId: string, newBillValue: number) => void; // 会計リセット成功時のコールバック
  onBalanceResetError?: (errorMsg: string) => void; // 会計リセット失敗時のコールバック
}
...
const UserDetailsModal: React.FC<UserDetailsModalProps> = ({
  ...
  const [loadingAction, setLoadingAction] = useState(false); // 承認・非承認などのアクション用
  const [isProcessingPayment, setIsProcessingPayment] = useState(false); // 現金受領処理専用のローディング

  // formatTimestamp関数をUserDetailsModal内に定義 (または共通ヘルパーからインポート)
  ...
  const handleCashPaymentAndResetBill = async () => {
    ...
    if ((user.bill ?? 0) === 0) {
      onUserUpdateSuccess?.("既にお支払い済みです。"); // 既に0円の場合はメッセージだけ表示
      return;
    }

    if (!window.confirm(`ユーザー「${user.pokerName || user.email}」の会計残高 ¥${(user.bill ?? 0).toLocaleString()} を現金で受け取り、残高を0円にしますか？`)) {
      return;
    }
    ...
    try {
      ...
      await updateDoc(userRef, {
        bill: 0,
        lastPaymentType: 'cash_reset', // 支払い方法の記録 (任意)
        lastPaymentAt: serverTimestamp(),   // 支払い日時の記録 (任意)
        updatedAt: serverTimestamp(),
      });
      onUserUpdateSuccess?.(`${user.pokerName || user.email} の会計残高を0にリセットしました。`);
      onBalanceResetSuccess?.(user.id, 0); // 親コンポーネントに通知 (ユーザーIDと新しい残高0を渡す)
      // モーダルを閉じるか、ユーザー情報を更新して表示を継続するかは呼び出し元の実装次第
      // onClose(); // この行を有効化すると、処理後にモーダルが自動で閉じる
    } catch (error: any) {
      console.error("会計残高のリセット処理に失敗:", error);
      onBalanceResetError?.(`会計残高のリセットに失敗: ${error.message}`);
    } finally {
    ...
    <div className="fixed inset-0 bg-slate-900 bg-opacity-75 flex items-center justify-center z-50 p-4 animate-fadeIn">
      <div className="bg-slate-800 p-6 rounded-lg shadow-xl w-full max-w-lg text-neutral-lightest max-h-[90vh] overflow-y-auto custom-scrollbar">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm mb-4">
          ...
          {/* ★★★ 会計残高表示をこちらに移動 ★★★ */}
          <div className="mt-2 pt-2 border-t border-slate-700 sm:col-span-2"> {/* sm:col-span-2 で幅を広げる */}
            <p className="text-slate-400">会計残高:</p>
            <p className="font-semibold text-yellow-400 text-xl">¥{(user.bill ?? 0).toLocaleString()}</p> {/* 少し大きく */}
          </div>
        ...
        {/* ★★★ 現金受領ボタンセクション ★★★ */}
        {(user.bill ?? 0) > 0 && ( // 残高がある場合のみ表示
          <div className="mt-6 pt-4 border-t border-slate-700">
            <h4 className="text-lg font-semibold text-neutral-lightest mb-2">会計処理</h4>
            <button
              onClick={handleCashPaymentAndResetBill}
              disabled={isProcessingPayment || loadingAction}
              className={`w-full px-4 py-2 rounded-md font-semibold transition-colors flex items-center justify-center
            ...
            >
              {isProcessingPayment ? (
                <><AiOutlineLoading className="inline-block animate-spin mr-2" size={18} />処理中...</>
              ) : `現金受領 (残高 ¥${(user.bill ?? 0).toLocaleString()} を0にする)`}
            </button>
          </div>
        )}
        {(user.bill ?? 0) === 0 && (
            <div className="mt-6 pt-4 border-t border-slate-700">
                <p className="text-green-400 text-center font-semibold">会計済みです。</p>
            </div>
        )}
